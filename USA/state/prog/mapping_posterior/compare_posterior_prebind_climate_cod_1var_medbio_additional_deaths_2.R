...
dat.merged.sub.year = ddply(dat.merged,.(cause,month,age,sex),summarise,deaths.added.two.deg.1=sum(deaths.added.two.deg.1),deaths.added.two.deg.2=sum(deaths.added.two.deg.2),
...
pdf(paste0(file.loc.git,country,'_additional deaths_w_error_',model,'_',model.2,'_',year.start,'_',year.end,'_',dname,'_',metric,'_',cause,'_fast_contig.pdf'),paper='a4r',height=0,width=0)
print(ggplot(data=dat.merged.sub.year,aes(x=deaths.added.two.deg.2,y=deaths.added.two.deg.1)) +
