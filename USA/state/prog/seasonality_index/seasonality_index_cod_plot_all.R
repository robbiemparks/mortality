...
plot.function.diff.seas.sig.5 <- function(data,shape.selected,x,y,n) {
    ...
    scale_x_continuous(name=paste0('Percent difference in death rates in ',year.start),labels=percent,limits=c(-5/100,(100/100))) +
    scale_y_continuous(name=paste0('Percent difference in death rates in ',year.end),labels=percent,limits=c(-5/100,(100/100))) +
...
plot.function.diff.seas.sig.5(lin.reg.grad.weight,17,.85,.2,3)
dev.off()
...
plot.function.diff.seas.sig.5(lin.reg.grad.weight.cardio,17,0.85,.2,2)
dev.off()
...
plot.function.diff.seas.sig.5(lin.reg.grad.weight.injury,17,0.85,.2,2)
dev.off()
