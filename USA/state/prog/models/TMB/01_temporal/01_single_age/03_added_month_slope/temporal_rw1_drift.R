...
parameters <- list(alpha_m=rep(1.,M),beta_0=1.,beta_m=rep(1.,M),log_mu=matrix(1.,N,T), pi=matrix(0.,N,T) ,log_prec_rw=1.,log_prec_epsilon=1.,log_prec_int_m=1.,log_prec_slp_m=1.)
...
#opt <- newton(par=obj$par,fn=objective$fn,gr=obj$gr,he=obj$he)
...
alpha_m <- extract.variable(sd,'alpha_m','fixed')
beta_m <- extract.variable(sd,'beta_m','fixed')
...
p1 <-   ggplot() +
        ...
        geom_ribbon(data=log_mu,alpha=0.2,fill='green',alpha_
